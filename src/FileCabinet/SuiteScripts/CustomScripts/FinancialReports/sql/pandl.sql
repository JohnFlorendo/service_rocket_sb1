...
SELECT 9 AS order_hide
	...
	, SUM(CASE WHEN TO_CHAR(ADD_MONTHS(CURRENT_DATE, -12), 'YYYY-MM') = TO_CHAR (transaction.trandate, 'YYYY-MM' )
	THEN ROUND( (NVL(transactionaccountingline.credit , 0) - NVL(transactionaccountingline.debit, 0)) * exchangerate.averagerate, 3)
	ELSE 0
	END) AS last_month
    , SUM(CASE WHEN TO_CHAR(ADD_MONTHS(CURRENT_DATE, -11), 'YYYY-MM') = TO_CHAR (transaction.trandate, 'YYYY-MM' )
	THEN ROUND( (NVL(transactionaccountingline.credit , 0) - NVL(transactionaccountingline.debit, 0)) * exchangerate.averagerate, 3)
...
INNER JOIN transactionaccountingline
	ON transactionLine.id = transactionaccountingline.transactionline
